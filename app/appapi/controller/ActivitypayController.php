...
class ActivitypayController extends HomebaseController {
	//支付宝 回调
	...
	*/
    ...
    /* 处理支付订单 */
    private function handelPay($where,$data=[]){
        $orderinfo=Db::name("activity_enroll")->where($where)->find();
        if(!$orderinfo){
            return 0;
        }
        ...
        $data['status']=$status;
        Db::name("activity_enroll")->where("id='{$orderinfo['id']}'")->update($data);

        Db::name("activity")->where("id='{$orderinfo['aid']}'")->inc('nums',1)->update();
	...
	protected function logali($msg){
		file_put_contents(CMF_ROOT.'data/log/activity_logali_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
    
	/* 打印log */
	protected function logwx($msg){
		file_put_contents(CMF_ROOT.'data/log/activity_logwx_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}

    /* 打印小程序支付log */
    protected function logsmall($msg){
        file_put_contents(CMF_ROOT.'data/log/activity_logsmall_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
    }

    /* 打印h5支付log */
    protected function loghfive($msg){
        file_put_contents(CMF_ROOT.'data/log/activity_loghfive_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
    }
