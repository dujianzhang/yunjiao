...
class ShareController extends HomebaseController{
    ...
    public function course() {
        ...
        $lessons=[];
        if($info['sort']==1){
            $lessons=Db::name('course_lesson')->field('id,uid,courseid,type,name,des,url,istrial,islive,starttime')->where('courseid',$info['id'])->order('list_order asc')->select()->toArray();

            foreach($lessons as $k1=>$v1){
