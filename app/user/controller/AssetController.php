...
class AssetController extends AdminBaseController
{
    ...
    public function webuploader()
    {
        if ($this->request->isPost()) {
            ...
            $cateWhere[] = ['type','=',$type[$arrData["filetype"]]];
            $cateWhere[] = ['pid','<>',0];
            $cateList = Db::name("resource_category")->where($cateWhere)->order("list_order asc")->select()->toArray();
            $this->assign([
                'cateList' =>$cateList,
            ]);
